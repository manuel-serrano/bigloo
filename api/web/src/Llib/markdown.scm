...
;*    Last change :  Fri Apr 10 07:52:28 2026 (serrano)                */
...
(module __web_markdown
   ...
   (export (markdown-parse::pair-nil ::input-port #!key (ctor list) charset fontifier)))
...
(define *quote-code-block-grammar*
   (regular-grammar ((line (+ (out "`\n,$")))
	 ...
	 (call-with-input-string str
      ...
      ((bol ",(")
       (rgc-buffer-unget-char (the-port) (char->integer #\())
       (set! lines (cons (ctor 'expr '() (read (the-port))) lines))
       (ignore))
      (else
...
(define *markdown-grammar*
   (regular-grammar ((punct (in "+=/-$~#%!'\""))
		     ...
		     (ident (: #\: (+ letter) (in " \t\n\r")))
		     (crlf (or "\n" "\r\n"))
		     ctor charset fontifier)
      
      (define (parse-string string charset fontifier)
       (call-with-input-string string
	  (lambda (ip)
	     (markdown-parse ip
		:ctor ctor :charset charset :fontifier fontifier))))
	  ...
	  (let* ((i (string-index str #\Newline))
		 (title (parse-string (substring str 0 i)
			   charset fontifier)))
	  ...
	  (lambda (e)
	     (exception-notify e)
	     (token 'ERROR e 0))
	  (token 'EXPR (read (the-port)) 0)))
      ;; embedded html
      ((: "<" (+ letter+) (or " " ">" "/>"))
       (let ((s (the-string)))
	  (rgc-buffer-insert-substring! (the-port) s 0 (string-length s))
	  (token 'HTML (html-parse (the-port)) 0)))
...
;*    markdown-parse ...                                               */
;*---------------------------------------------------------------------*/
(define (markdown-parse::pair-nil ip::input-port #!key (ctor list) charset fontifier)
   ...
   (define (peek-token)
	  ...
	  (begin
	     (set! *peeked-tokens*
		(list (read/rp *markdown-grammar* ip ctor charset fontifier)))
	     (car *peeked-tokens*))
	  (car *peeked-tokens*)))
   ...
   (define (parse-span-elements state::MDState tag::procedure end bra-as-text o-as-text)
      (cond
	 ...
	 ((and bra-as-text (memq (peek-token-type) '(OBRA CBRA)))
	  ...
	  (parse-span-elements state tag end bra-as-text o-as-text))
   ...
   (define (parse-span state::MDState tag::procedure end cleanup
	      #!key wrapper bra-as-text o-as-text)
      (let ((sstate::MDState (instantiate::MDState
				(tag 'span)
				(wrapper wrapper))))
	 (multiple-value-bind (retcode val)
	    (parse-span-elements sstate tag end bra-as-text o-as-text)
	    (case retcode
	       ((span)
		(state-add! state val))
	       ((unwind)
   ...
   (define (end-subblock state::MDState)
      (let ((el (ctor (-> state tag) (-> state attributes) (reverse! (-> state elements)))))
      ...
      (let ((hr (case (peek-token-type)
		   ((IDCLA)
		    (let ((v (token-value (consume-any!))))
		       (ctor 'HR (list :id (car v) :class) (cdr v))))
		   (else
		    (ctor 'HR '() '())))))
   ...
   (define (section state::MDState)
      (let* ((tok (consume-any!))
	     ...
	     (title (if (string? (cdr sec))
			(conv (cdr sec))
			(cdr sec)))
	     (id #f))
	 (tprint "TOK=" (format "~s" tok) " TITLE=" (format "~s" title))
	 (if (symbol? tag)
	     (let* ((s (case (peek-token-type)
			  ((IDCLA)
			   (let ((v (token-value (consume-any!))))
			      (set! id (or (car v) (symbol->string (gensym))))
			      (ctor tag (list :id id :class (cdr v)) title)))
			  (else
			   (set! id (symbol->string (gensym)))
			   (ctor tag (list :id id) title)))))
		(if (pair? (-> state elements))
   ...
   (define (prog state::MDState lang-id)
	 ...
	 (let* ((lines (read/rp *quote-code-block-grammar* ip '() conv ctor #f))
		(body (cond
			 ((string-null? lang)
			  lines)
			 ((null? lines)
   ...
   (define (reference state::MDState ctor)
	 ...
	 (if (span lstate)
		...
		(let loop ()
		   (case (peek-token-type)
			  ...
			  (consume-token! 'CPAR)
			  (state-add! state
			     (ctor 'URL (list :url url :title title)
		      ...
		      ((OBRA)
		       (let* ((token (consume-any!))
			      (ref (lref))
			      (el (ctor 'LINK '() (reverse! (-> lstate elements)))))
			  (set! unresolved-refs
   ...
   (define (span state::MDState)
	 ...
	 ((URL)
	  (let ((url (conv (token-value (consume-any!)))))
	     (state-add! state (ctor 'A (list :url url) url)))
	  #t)
	 ((O_)
	  (let ((val (token-value (consume-any!))))
	     (parse-span state (lambda l (ctor 'EM '() l)) 'O_ val))
	  #t)
	 ((O*)
	  (let ((val (token-value (consume-any!))))
	     (parse-span state (lambda l (ctor 'EM '() l)) 'O* val))
	  #t)
	 ((O__)
	  (let ((val (token-value (consume-any!))))
	     (parse-span state (lambda l (ctor 'STRONG '() l)) 'O__ val))
	  #t)
	 ((O**)
	  (let ((val (token-value (consume-any!))))
	     (parse-span state (lambda l (ctor 'STRONG '() l)) 'O** val))
	  #t)
	 ((CODE)
	  (let ((val (token-value (consume-any!))))
	     (if (eq? (-> state wrapper) 'CODE2)
		 (state-add! state "`")
		 (parse-span state (lambda l (ctor 'CODE '() l)) 'CODE val
		    :bra-as-text #t
		    :o-as-text #t)))
	  #t)
	 ((CODE2)
	  (let ((val (token-value (consume-any!))))
	     (parse-span state (lambda l (ctor 'CODE '() l)) 'CODE2 val
		:wrapper 'CODE2
		:bra-as-text #t
		:o-as-text #t))
	  #t)
   ...
   (define (block state::MDState bob)
      (tprint "BLOCK " (peek-token-type) " " (peek-token-value))
      (cond
