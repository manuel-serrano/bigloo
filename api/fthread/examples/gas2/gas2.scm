;*=====================================================================*/
;*    .../prgm/project/bigloo/api/fthread/examples/gas2/gas2.scm       */
;*    -------------------------------------------------------------    */
;*    Author      :  Manuel Serrano                                    */
;*    Creation    :  Mon Feb  4 14:28:58 2002                          */
;*    Last change :  Mon Jun 29 09:51:41 2009 (serrano)                */
...
(define (atom-initialize! a::atom)
   ...
   (with-access::atom a (color %ci oldx oldy)
...
(define (add-atoms num)
   (define (add-atom atom)
	 ...
	 (if (not (eq? atom atom2))
		...
		;; at the same instant
		(if (and (not (eq? color *color*))
			 (eq? (atom-color atom2) *color*))
		    (begin
		       (set! *ill-atom-number* (+fx 1 *ill-atom-number*))
		       (refresh-status-gauge!)
		       (set! color *color*)
			  ...
			  (set! color *color*)
			  (set! outline *color*))))
		;; after each collision, we select a new direction for the atom
		(atom-direction-set! atom))))
      (define (move)
