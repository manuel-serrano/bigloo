	 ...
	 (cond
	    ...
	    (else
	     ...
	     (let* ((type (declare-subtype! id name '(obj) 'bigloo))
		    (pred (symbol-append id '?)))
		(add-coercion! type *obj*
		   (list (cons #t type)) (list (cons #t *obj*)))
		(add-coercion! *obj* type
		   (list (cons pred *obj*)) (list (cons #t type)))
...
(define-method (make-ctype-accesses! what::jarray who::type loc module::symbol)
   (with-trace 'jvm "make-ctype-accesses ::jarray"
	 ...
	 (for-each (lambda (proto)
		      (statexp-parser proto 'static module))
	    `((inline ,id->obj ,tid)
	      (inline ,tobj->id ::obj)
	      (inline ,tid? ::obj)
