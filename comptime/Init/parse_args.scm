...
(define (do-parse-args args)
   ...
   (pass-args-parse (lib-dir jvm wasm default) default args
...
;*--- misc ------------------------------------------------------------*/
      ...
      (("-cache-clean" (help "Cleanup module cache and exit"))
       (when (directory? *module-cache-dir*)
	  (print "cleaning directory \"" *module-cache-dir* "\"...")
...
;*--- Optimization ----------------------------------------------------*/
      ...
      (("-fno-saw-bbv" (help "Disable saw basic-blocks versionning"))
       (set! *saw-bbv?* #f))
      (("-fsaw-bbv-fun" ?name (help "Apply bbv on these functions"))
       (set! *saw-bbv?* #t)
...
;*--- Compiler stages -------------------------------------------------*/
      ...
      (("-nums" (help "Stop after the nums stage"))
       (set! *pass* 'nums))
      (("-fastfl" (help "Stop after the fastfl stage"))
       (set! *pass* 'fastfl))
