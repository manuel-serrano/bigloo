...
(define (dot-set->node::node get::getfield val stack loc site genv)
   (with-access::getfield get (expr* fid ftype otype)
	 ...
	 (if (slot-read-only? slot)
		...
		exp loc genv)
	     (make-field-set! slot (car expr*) val stack loc site genv)))))
...
;*    make-field-ref ...                                               */
;*---------------------------------------------------------------------*/
(define (make-field-ref slot obj stack loc site genv)
   (if (slot-getter slot)
...
;*    make-field-set! ...                                              */
;*---------------------------------------------------------------------*/
(define (make-field-set! slot obj val stack loc site genv)
   (cond
      ((slot-setter slot)
       (let* ((vnum (slot-virtual-num slot))
