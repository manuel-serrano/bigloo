...
(define-method (node->rtl::area e::pragma) ; ()
   (with-access::pragma e (type expr* format srfi0)
      (if (and (string-null? format)
	       (pair? expr*)
	       (null? (cdr expr*))
	       (isa? (car expr*) var))
	  (let ((fmt (variable-name (var-variable (car expr*)))))
	     (call* e (instantiate::rtl_pragma (type type) (format fmt) (srfi0 srfi0)) (cdr expr*)))
	  (call* e (instantiate::rtl_pragma (type type) (format format) (srfi0 srfi0)) expr*))) )
