...
(define (install-type-cache!)
   ...
   (set! *s8vector* (use-type! 's8vector #f))
   (set! *u8vector* (use-type! 'u8vector #f))
   (set! *s16vector* (use-type! 's16vector #f))
   (set! *u16vector* (use-type! 'u16vector #f))
   (set! *s32vector* (use-type! 's32vector #f))
   (set! *u32vector* (use-type! 'u32vector #f))
   (set! *s64vector* (use-type! 's64vector #f))
   (set! *u64vector* (use-type! 'u64vector #f))
   (set! *f32vector* (use-type! 'f32vector #f))
   (set! *f64vector* (use-type! 'f64vector #f))
   (set! *hvectors* (list *s8vector* *u8vector*
		       *s16vector* *u16vector* 
		       *s32vector* *u32vector* 
		       *s64vector* *u64vector*
		       *f32vector* *f64vector*))
...
(define *s8vector* 'no-type-yet)
(define *u8vector* 'no-type-yet)
(define *s16vector* 'no-type-yet)
(define *u16vector* 'no-type-yet)
(define *s32vector* 'no-type-yet)
(define *u32vector* 'no-type-yet)
(define *s64vector* 'no-type-yet)
(define *u64vector* 'no-type-yet)
(define *f64vector* 'no-type-yet)
(define *f32vector* 'no-type-yet)
