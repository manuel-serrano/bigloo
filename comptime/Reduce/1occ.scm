;*=====================================================================*/
;*    serrano/prgm/project/bigloo/5.0.x/comptime/Reduce/1occ.scm       */
;*    -------------------------------------------------------------    */
;*    Author      :  Manuel Serrano                                    */
;*    Creation    :  Thu Jul 13 10:29:17 1995                          */
;*    Last change :  Sat Jun 13 08:25:47 2026 (serrano)                */
...
(define (reduce-1occ! globals)
   ...
   (for-each (lambda (global)
		(let* ((fun  (global-value global))
		       (node (sfun-body fun)))
		   (sfun-body-set! fun (bind-values (_ node)
					  (node-1occ! node '())
					  node))
		   #unspecified))
	     globals)
...
(define-method (node-1occ! node::sync 1-exp*)
   (with-access::sync node (body mutex prelock)
      (bind-values (resetm nmutex)
	 (node-1occ! mutex 1-exp*)
	 (set! mutex nmutex)
	 (bind-values (resetp nprelock)
...
(define-method (node-1occ! node::app-ly 1-exp*)
   (with-access::app-ly node (fun arg)
      (bind-values (reset nfun)
	 (node-1occ! fun 1-exp*)
	 (set! fun nfun)
	 (bind-values (reset' narg)
...
(define-method (node-1occ! node::conditional 1-exp*)
   (with-access::conditional node (test true false)
      (bind-values (reset ntest)
	 (node-1occ! test 1-exp*)
	 (set! test ntest)
	 (let ((1-exp*' (if reset '() 1-exp*)))
	    (bind-values (reset' ntrue)
	       (node-1occ! true 1-exp*')
	       (set! true ntrue)
	       (bind-values (reset'' nfalse)
...
(define-method (node-1occ! node::fail 1-exp*)
   (with-access::fail node (proc msg obj)
      (bind-values (reset nproc)
	 (node-1occ! proc 1-exp*)
	 (set! proc nproc)
	 (let ((1-exp*' (if reset '() 1-exp*)))
	    (bind-values (reset' nmsg)
	       (node-1occ! msg 1-exp*')
	       (set! msg nmsg)
	       (let ((1-exp*'' (if reset' '() 1-exp*')))
		  (bind-values (reset'' nobj)
		     (node-1occ! obj 1-exp*'')
		     (set! obj nobj)
		     (values (or reset reset' reset'') node))))))))
