...
(defvar tags-syntax-table nil
  "Etags get confused with syntax table. It evaluates the search for
tags in the tags buffer. This buffer makes use of the fundamental-mode
and thus, does not uses the correct syntax table (regarding the mode of the
buffer that is callings tags). To fix this, we have introduced this variable
that holds the caller buffer syntax table.")

(defvar tags-auto-reload-p t
  "Reload new TAGS table when updated without user request.")
...
(defun find-tag-interactive (prompt &optional no-default)
  (setq tags-syntax-table (syntax-table))
...
;;;###autoload
(defun find-tag-other-frame (tagname &optional next-p regexp-p)
  "Find tag (in current tags table) whose name contains TAGNAME.
