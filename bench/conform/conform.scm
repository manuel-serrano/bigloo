...
(define (make-edge-setter mutator!)
  (lambda (node value)
    (cond ((any-node? node)
	   (error '() '() "Can't set edges from the ANY node"))
	  ((none-node? node) 'OK)
...
; Selectors
(define operation (lambda (x) (blue-edge-operation x)))
(define arg-node  (lambda (x) (blue-edge-arg-node x)))
(define res-node  (lambda (x) (blue-edge-res-node x)))

; Mutators
(define set-arg-node! (lambda (x y) (set-blue-edge-arg-node! x y)))
(define set-res-node! (lambda (x y) (set-blue-edge-res-node! x y)))
...
(define graph-nodes (lambda (x) (internal-graph-nodes x)))
(define already-met (lambda (x) (internal-graph-already-met x)))
(define already-joined (lambda (x) (internal-graph-already-joined x)))
	   ...
	   (lambda (blue-edge)
	     (set-arg-node! blue-edge
			    (find-canonical-representative
			     (arg-node blue-edge) classes))
	     (set-res-node! blue-edge
			    (find-canonical-representative
			     (res-node blue-edge) classes)))
	   (blue-edges node))))
    node)
  (define (fix-table table)
    (define (canonical? node)
       (eq? node (find-canonical-representative node classes)))
...
(define (make-lattice g print?)
  ...
  (let ((graph
	 (apply make-graph
		(adjoin any-node
			(adjoin none-node (graph-nodes (clean-graph g)))))))
...
(define (main argv)
   (let loop ((i (string->number (cadr argv))))
      (if (= i 1)
	  (begin
	     (display "CONFORM")
	     (display (test))
