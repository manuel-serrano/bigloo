...
#   define BGL_VECTORP(c) BGL_TAGGED_PTRP(c, TAG_VECTOR, TAG_MASK)
#else
...
#define BGL_VECTOR_BLIT(fun, dest, src, destidx, srcidx, size) \
   (fun(&(VECTOR_REF(dest, destidx)), &(VECTOR_REF(src, srcidx)), size * sizeof(obj_t)), dest)

#define BGL_VECTOR_BLIT_NO_OVERLAP(dest, src, destidx, srcidx, size) \
   BGL_VECTOR_BLIT(memcpy, dest, src, destidx, srcidx, size)
#define BGL_VECTOR_BLIT_OVERLAP(dest, src, destidx, srcidx, size) \
   BGL_VECTOR_BLIT(memmove, dest, src, destidx, srcidx, size)
...
/*---------------------------------------------------------------------*/
#if (BGL_HAVE_ALLOCA && (BGL_TAGGING != BGL_TAGGING_NAN) && defined(__GNUC__))
#  if(!defined(TAG_VECTOR))
