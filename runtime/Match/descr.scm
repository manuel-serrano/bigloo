...
(define (pattern-variables f)
   (cond 
      ...
      ((eq? (car f) 'var)
       ;; (var name) or (var name pattern) — return just the name
       (list (cadr f)))
      ((eq? (car f) 'vector-begin)
       (pattern-variables (caddr f)) )
      ((eq? (car f) 'vector-end)
       '())
      ((eq? (car f) 'ssetq-append)
       ;; (ssetq-append name tree-pattern tail-pattern)
       (union (list (cadr f))
	      (union (pattern-variables (caddr f))
		     (pattern-variables (cadddr f)))))
      ((eq? (car f) 'eval-append)
       ;; (eval-append name tail-pattern)
       (pattern-variables (caddr f)))
      ((eq? (car f) 'end-ssetq)
       '())
      ((eq? (car f) 'class-pat)
       ;; (class-pat name (field-name . pattern) ...)
       (let loop ((aps (cddr f)))
          (if (null? aps)
              '()
              (union (pattern-variables (cdar aps))
                     (loop (cdr aps))))))
      ((eq? (car f) 'struct-pat)
