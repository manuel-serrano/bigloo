...
(define (comp-old-call e stk)
   (with-access::ev_app e (loc fun args tail?)
	 ...
	 (let ( (args (comp-with-push args stk)) )
	    (EVA (call huge) ()
		 (let ( (f (EVC f)) )
		    (unless (procedure? f)
...
(define-macro (generate-comp-calli-body tail? args)
   ...
   `(EVA (call normal ,(length args)) ("nb args" ,(length args) "size" size)
	 (let ( (f (EVC fun)) ,@(map (lambda (v) `(,v (EVC ,v))) args) )
	    ...
	    (let ( (uf (procedure-attr f)) )
		   ...
		   (let ( (arity (user-arity uf)) (run (user-runner uf)) (sf (user-frame uf)) )
			 ...
			 ((,(length args))
			  ,(push-args-tail? args) )
			 ,@(generate-case-neg-arity -1 (-fx -2 (length args)) args)
			 (else
			  (error 'eval "wrong number of arguments" (cons arity ,(length args))) ))))))))
...
(define (bind-frame s arity vals)
   (error 'eval "bind-frame" 'no-more) )
		  ...
		  (let ( (main (lambda (,@param . ,extra)
				  ...
				  (let ( (s (find-state)) )
					...
					,@(cond
					     ...
					     (else `((bind-frame s (correct ,arity ,nbparam)
								 ,extra))) )
