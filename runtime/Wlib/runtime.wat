...
;*    Last change :  Mon Dec 30 06:52:03 2024 (serrano)                */
...
;* 	 (then (return_call $bgl_bexception_handler                    */
;* 		  (ref.cast (ref $bexception) (local.get $exn))        */
;* 		  (ref.cast (ref $exit) (local.get $protect))))        */
;* 	 (else (call $js_trace (i32.const 555554))                     */
;* 	       (return (global.get $BUNSPEC)))))                       */
  ...
  (func $BGL_ENV_ERROR_HANDLER_SET (export "BGL_ENV_ERROR_HANDLER_SET") 
    (param $env (ref $dynamic-env))
    (param $hdl (ref eq)) 
  ...
  (func $BGL_ENV_ERROR_HANDLER_PUSH (export "BGL_ENV_ERROR_HANDLER_PUSH")
     (param $env (ref $dynamic-env))
     (param $h (ref eq))
     (param $hdl (ref eq))
     (return_call $BGL_ENV_ERROR_HANDLER_SET
