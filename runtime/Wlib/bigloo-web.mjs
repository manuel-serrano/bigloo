...
const stdioBuf = [ null, null, null ];

function flushStdio(fd, str) {
   ...
   if (j < 0) {
	 ...
	 return null;
      ...
      const r = stdioBuf[fd].concat(str.substring(0, j - 1));
      if (j === str.length - 1) {
	 stdioBuf[fd] = null;
      } else {
	 stdioBuf[fd] = str.substring(j+1, str.length);
      }
      return r;
...
/*    BglWebRuntime                                                    */
/*---------------------------------------------------------------------*/
class BglWebRuntime extends BglRuntime {
...
/*    WasmInstantiate ...                                              */
/*---------------------------------------------------------------------*/
async function WasmInstantiate(wasm, jsenv) {
   const f = await fetch(wasm);
   const buf = await f.arrayBuffer();
   const mach = await WebAssembly.instantiate(buf, jsenv);
