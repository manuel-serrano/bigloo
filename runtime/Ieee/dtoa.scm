...
;*    Last change :  Tue Jul  1 06:57:10 2025 (serrano)                */
...
(define-macro (static v)
   ...
   (define (int-vector v)
      (let ((l (vector->list v)))
	 `(static-pragma ,(format "long __dividers_e[] = { ~(, ) };" l))))
   
   (define (llong-vector v)
      (let ((l (vector->list v)))
	 `(static-pragma ,(format "BGL_LONGLONG_T __dividers_v[] = { ~(, ) };" l))))
   ...
   (cond-expand
      ...
      (else
       (if (llong? (vector-ref (cadr v) 0))
	   `(define __dividers_v ',(cadr v))
	   `(define __dividers_e ',(cadr v))))))
...
;*    dividers-v-ref ...                                               */
;*---------------------------------------------------------------------*/
(define-inline (dividers-v-ref::llong idx::long)
   (cond-expand
      (bigloo-c (pragma::llong "__dividers_v[$1]" idx))
      (else (vector-ref __dividers_v idx))))

;*---------------------------------------------------------------------*/
;*    dividers-e-ref ...                                               */
;*---------------------------------------------------------------------*/
(define-inline (dividers-e-ref::long idx::long)
   (cond-expand
      (bigloo-c (pragma::long "__dividers_e[$1]" idx))
      (else (vector-ref __dividers_e idx))))
...
(define (cached-divider k)
   (let* ((index (+fx (dividers-offset) k))
	  (v (dividers-v-ref index))
	  (e (dividers-e-ref index))
