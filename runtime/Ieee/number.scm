...
(module __r4_numbers_6_5
   ...
   (extern  (macro $fixnum->flonum::double (::long)   "(double)")
	    ...
	    (macro $flonum->llong::llong   (::double) "DOUBLE_TO_LLONG")
   ...
   (export  (number?::bool                  obj)
	    ...
	    (number->string::string         x #!optional (radix 10))
	    (string->number                 x #!optional (radix 10)))

   (pragma  ($fixnum->flonum side-effect-free args-safe (effect))
	    ($flonum->fixnum side-effect-free args-safe (effect))
	    ($elong->flonum side-effect-free args-safe (effect))
	    ($flonum->elong side-effect-free args-safe (effect))
	    ($llong->flonum side-effect-free args-safe (effect))
	    ($flonum->llong side-effect-free args-safe (effect))
	    ...
	    (2max side-effect-free (effect))
	    (min side-effect-free (effect))
	    (2min side-effect-free (effect))
	    (2+ side-effect-free (effect))
	    (+ side-effect-free (effect))
	    (2* side-effect-free (effect))
	    (* side-effect-free (effect))
...
;*    fixnum->byte ...                                                 */
;*---------------------------------------------------------------------*/
(define-inline (fixnum->byte x)
   ($long->byte x))

;*---------------------------------------------------------------------*/
;*    fixnum->ubyte ...                                                */
;*---------------------------------------------------------------------*/
(define-inline (fixnum->ubyte x)
   ($long->ubyte x))

;*---------------------------------------------------------------------*/
;*    byte->fixnum ...                                                 */
;*---------------------------------------------------------------------*/
(define-inline (byte->fixnum x)
   ($byte->long x))

;*---------------------------------------------------------------------*/
;*    ubyte->fixnum ...                                                */
;*---------------------------------------------------------------------*/
(define-inline (ubyte->fixnum x)
   ($ubyte->long x))
