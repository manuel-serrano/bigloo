...
/*    Last change :  Fri Nov 15 06:44:12 2024 (serrano)                */
...
static obj_t
alloc_make_unsafe_cell(obj_t val) {
   obj_t unsafe_cell;

   unsafe_cell = (obj_t)GC_MALLOC(BGL_UNSAFE_CELL_SIZE);
...
bgl_make_unsafe_cell(obj_t val) {
   ...
   GC_INLINE_MALLOC(unsafe_cell, BGL_UNSAFE_CELL_SIZE, alloc_make_unsafe_cell(val));
   BGL_UNSAFE_CELL_SET(unsafe_cell, val);
   return unsafe_cell;
}
...
alloc_make_belong(long l) {
   ...
   elong->elong.header = BGL_MAKE_HEADER(ELONG_TYPE, ELONG_SIZE);
   elong->elong.val = l;
...
alloc_make_bllong(BGL_LONGLONG_T l) {
   ...
   llong->llong.header = BGL_MAKE_HEADER(LLONG_TYPE, LLONG_SIZE);
   llong->llong.val = l;
...
alloc_make_bint32(int32_t l) {
   ...
   int32->sint32.header = BGL_MAKE_HEADER(INT32_TYPE, BGL_INT32_SIZE);
   int32->sint32.val = l;
...
alloc_make_buint32(uint32_t l) {
   ...
   int32->uint32.header = BGL_MAKE_HEADER(UINT32_TYPE, BGL_UINT32_SIZE);
   int32->uint32.val = l;
...
alloc_make_bint64(int64_t l) {
   ...
   int64->sint64.header = BGL_MAKE_HEADER(INT64_TYPE, BGL_INT64_SIZE);
   int64->sint64.val = l;
...
alloc_make_buint64(uint64_t l) {
   ...
   int64->uint64.header = BGL_MAKE_HEADER(UINT64_TYPE, BGL_UINT64_SIZE);
   int64->uint64.val = l;
