...
/*    Last change :  Sat Nov  9 21:27:56 2024 (serrano)                */
/*    -------------------------------------------------------------    */
...
/*---------------------------------------------------------------------*/
obj_t
bgl_open_input_descriptor(int fd, obj_t buffer) {
   FILE *file = fdopen(fd, "r");

   if (!file) {
      return BFALSE;
   } else {
      obj_t port = bgl_make_input_port(BINT(fd), file, KINDOF_FILE, buffer);
      struct stat _stati;
      ...
      /* file size */
      if (fstat(fd, &_stati)) {
	 BGL_INPUT_PORT_LENGTH_SET(port, (long)_stati.st_size);
      } else {
	 BGL_INPUT_PORT_LENGTH_SET(port, -1);
      }
