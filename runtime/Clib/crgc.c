/*=====================================================================*/
/*    serrano/prgm/project/bigloo/wasm/runtime/Clib/crgc.c             */
/*    -------------------------------------------------------------    */
/*    Author      :  Manuel Serrano                                    */
/*    Creation    :  Sun Sep 13 11:58:32 1998                          */
/*    Last change :  Tue Oct  1 08:38:04 2024 (serrano)                */
/*    -------------------------------------------------------------    */
...
rgc_fill_buffer(obj_t port) {
   if (INPUT_PORT_CLOSEP(port)) {
	 ...
	 char *buf = (char *)&RGC_BUFFER_REF(port, 0);
	 ...
	 fill:
	    ...
	    if (INPUT_PORT(port).matchstart > 0) {
	       ...
	       buf = (char *)&RGC_BUFFER_REF(port, 0);
...
rgc_reserve_space(obj_t port, long amount) {
   ...
   char *buffer = (char *)&RGC_BUFFER_REF(port, 0);
   ...
   if ((matchstop + (bufsize - bufpos)) >= amount) {
      ...
      memmove(&RGC_BUFFER_REF(port, amount),
	      &RGC_BUFFER_REF(port, matchstop),
	      bufpos - matchstop);
...
rgc_buffer_insert_substring(obj_t ip, obj_t str, long from, long to) {
   ...
   memmove(&RGC_BUFFER_REF(ip, matchstop - len),
	   &STRING_REF(str, from),
	   len);
