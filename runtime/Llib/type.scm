...
;*     (coerce nil list           ()               ())                 */
   ...
   (pragma
    ($bool->bbool side-effect-free nesting args-safe fail-safe (effect))
    ($bbool->bool side-effect-free nesting args-safe fail-safe (effect))
    ($obj->bool side-effect-free nesting args-safe fail-safe (effect))
    ($byte->bint side-effect-free nesting args-safe fail-safe (effect))
    ($ubyte->bint side-effect-free nesting args-safe fail-safe (effect))
    ($short->bint side-effect-free nesting args-safe fail-safe (effect))
    ($ushort->bint side-effect-free nesting args-safe fail-safe (effect))
    ($int->bint side-effect-free nesting args-safe fail-safe (effect))
    ($uint->bint side-effect-free nesting args-safe fail-safe (effect))
    ($long->bint side-effect-free nesting args-safe fail-safe (effect))
    ($ulong->bint side-effect-free nesting args-safe fail-safe (effect))
    ($bint->byte side-effect-free nesting args-safe fail-safe (effect))
    ($bint->ubyte side-effect-free nesting args-safe fail-safe (effect))
    ($bint->short side-effect-free nesting args-safe fail-safe (effect))
    ($bint->ushort side-effect-free nesting args-safe fail-safe (effect))
    ($bint->int side-effect-free nesting args-safe fail-safe (effect))
    ($bint->uint side-effect-free nesting args-safe fail-safe (effect))
    ($bint->long side-effect-free nesting args-safe fail-safe (effect))
    ($bint->ulong side-effect-free nesting args-safe fail-safe (effect))
    ($ubyte->short side-effect-free nesting args-safe fail-safe (effect))
    ($ubyte->ushort side-effect-free nesting args-safe fail-safe (effect))
    ($ubyte->int side-effect-free nesting args-safe fail-safe (effect))
    ($ubyte->uint side-effect-free nesting args-safe fail-safe (effect))
    ($ubyte->long side-effect-free nesting args-safe fail-safe (effect))
    ($ubyte->ulong side-effect-free nesting args-safe fail-safe (effect))
    ($byte->short side-effect-free nesting args-safe fail-safe (effect))
    ($byte->ushort side-effect-free nesting args-safe fail-safe (effect))
    ($byte->int side-effect-free nesting args-safe fail-safe (effect))
    ($byte->uint side-effect-free nesting args-safe fail-safe (effect))
    ($byte->long side-effect-free nesting args-safe fail-safe (effect))
    ($byte->ulong side-effect-free nesting args-safe fail-safe (effect))
    ($short->byte side-effect-free nesting args-safe fail-safe (effect))
    ($short->ubyte side-effect-free nesting args-safe fail-safe (effect))
    ($short->ushort side-effect-free nesting args-safe fail-safe (effect))
    ($short->int side-effect-free nesting args-safe fail-safe (effect))
    ($short->uint side-effect-free nesting args-safe fail-safe (effect))
    ($short->long side-effect-free nesting args-safe fail-safe (effect))
    ($short->ulong side-effect-free nesting args-safe fail-safe (effect))
    ($ushort->byte side-effect-free nesting args-safe fail-safe (effect))
    ($ushort->ubyte side-effect-free nesting args-safe fail-safe (effect))
    ($ushort->short side-effect-free nesting args-safe fail-safe (effect))
    ($ushort->int side-effect-free nesting args-safe fail-safe (effect))
    ($ushort->uint side-effect-free nesting args-safe fail-safe (effect))
    ($ushort->long side-effect-free nesting args-safe fail-safe (effect))
    ($ushort->ulong side-effect-free nesting args-safe fail-safe (effect))
    ($int->byte side-effect-free nesting args-safe fail-safe (effect))
    ($int->ubyte side-effect-free nesting args-safe fail-safe (effect))
    ($int->short side-effect-free nesting args-safe fail-safe (effect))
    ($int->ushort side-effect-free nesting args-safe fail-safe (effect))
    ($int->uint side-effect-free nesting args-safe fail-safe (effect))
    ($int->long side-effect-free nesting args-safe fail-safe (effect))
    ($int->ulong side-effect-free nesting args-safe fail-safe (effect))
    ($uint->byte side-effect-free nesting args-safe fail-safe (effect))
    ($uint->ubyte side-effect-free nesting args-safe fail-safe (effect))
    ($uint->short side-effect-free nesting args-safe fail-safe (effect))
    ($uint->ushort side-effect-free nesting args-safe fail-safe (effect))
    ($uint->int side-effect-free nesting args-safe fail-safe (effect))
    ($uint->long side-effect-free nesting args-safe fail-safe (effect))
    ($uint->ulong side-effect-free nesting args-safe fail-safe (effect))
    ($long->byte side-effect-free nesting args-safe fail-safe (effect))
    ($long->ubyte side-effect-free nesting args-safe fail-safe (effect))
    ($long->short side-effect-free nesting args-safe fail-safe (effect))
    ($long->ushort side-effect-free nesting args-safe fail-safe (effect))
    ($long->int side-effect-free nesting args-safe fail-safe (effect))
    ($long->uint side-effect-free nesting args-safe fail-safe (effect))
    ($long->ulong side-effect-free nesting args-safe fail-safe (effect))
    ($ulong->byte side-effect-free nesting args-safe fail-safe (effect))
    ($ulong->ubyte side-effect-free nesting args-safe fail-safe (effect))
    ($ulong->short side-effect-free nesting args-safe fail-safe (effect))
    ($ulong->ushort side-effect-free nesting args-safe fail-safe (effect))
    ($ulong->int side-effect-free nesting args-safe fail-safe (effect))
    ...
    ($uchar->char side-effect-free nesting args-safe fail-safe (effect))
    ($bchar->char side-effect-free nesting args-safe fail-safe (effect))
    ($bchar->uchar side-effect-free nesting args-safe fail-safe (effect))
    ($ucs2->bucs2 side-effect-free nesting args-safe fail-safe (effect))
    ($bucs2->ucs2 side-effect-free nesting args-safe fail-safe (effect))
    ($double->real side-effect-free args-safe fail-safe (effect))
    ($real->double side-effect-free args-safe fail-safe (effect))
    ($double->float side-effect-free nesting args-safe fail-safe (effect))
    ...
    ($char->ubyte side-effect-free nesting args-safe fail-safe (effect))
