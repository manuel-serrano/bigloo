...
public class output_buffered_port extends output_port {
   ...
   public output_buffered_port(final byte[] file, byte[] _buffer, final boolean append)
      throws IOException {
      super(append);
   ...
   public output_buffered_port(final byte[] file, byte[] _buffer)
      throws IOException {
      super(file);
      out = new RandomAccessFile(file, "w");
      buffer = _buffer;
   ...
   public static output_port make_output_buffered_port(final byte[] file, byte[] buffer)
      throws IOException {
      if (buffer == null || buffer.length == 0) {
