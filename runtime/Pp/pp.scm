...
;*    Last change :  Tue Apr 28 07:51:10 2026 (serrano)                */
;*    -------------------------------------------------------------    */
...
(module __pp
   ...
   (export (pp::unspecified exp #!optional (port::output-port (current-output-port)) #!key (width::long 80) (case-sensitivity::symbol 'respect))))
...
;*    pp ...                                                           */
;*---------------------------------------------------------------------*/
(define (pp exp #!optional (port::output-port (current-output-port)) #!key (width::long 80) (case-sensitivity::symbol 'respect))
   (generic-write exp #f width case-sensitivity (lambda (s) (display s port) #t))
   #unspecified)
...
(define (generic-write obj display? width case-sensitivity output)
   ...
   (define (pp obj col)
      ...
      (define (pr obj col extra pp-pair)
         (if (or (pair? obj) (vector? obj)) ; may have to split on multiple lines
             (let ((result '())
                ...
                (generic-write obj display? #f case-sensitivity
                   (lambda (str)
      ...
      (define (style head)
         (case (if (eq? case-sensitivity 'respect)
...
(define (object->string obj)
  (let ((result '()))
    (generic-write obj #f #f 'respect (lambda (str) (set! result (cons str result)) #t))
...
(define (pretty-print obj . opt)
  (let ((port (if (pair? opt) (car opt) (current-output-port))))
    (generic-write obj #f 79 'respect (lambda (s) (display s port) #t))))
...
(define (pretty-print-to-string obj)
  (let ((result '()))
    (generic-write obj #f 79 'respect (lambda (str) (set! result (cons str result)) #t))
    (reverse-string-append result)))
