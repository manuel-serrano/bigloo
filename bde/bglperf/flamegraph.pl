...
USAGE: $0 [options] infile > outfile.svg\n
	...
	--colors PALETTE # set color palette. choices are: hot (default), mem,
	                 ...
	                 # aqua, yellow, purple, orange, bigloo
...
sub color {
	...
	# multi palettes
	if (defined $type and $type eq "bigloo") {
		# Handle both annotations (_[j], _[i], ...; which are
		# accurate), as well as input that lacks any annotations, as
		# best as possible. Without annotations, we get a little hacky
		# and match on java|org|com, etc.
		if ($name =~ /^GC_/) {	# GC functions
			$type = "gc";
		} elsif ($name =~ /^bgl_/) {	# C Bigloo runtime
			$type = "green";
		} elsif ($name =~ /\@__hopscript/) { # hopscript compiled functions
			$type = "purple";
		} elsif ($name =~ /[@][^@]/) {	# Bigloo compiled functions
			$type = "orange";
		} elsif ($name =~ /(pthread|futex|mutex)/) {	# multi-threading
			$type = "thread";
		} elsif ($name =~ /unknown/) {	# untraced functions
			$type = "unknown";
		} else {			# system
			$type = "aqua";
		}
	...
	if (defined $type and $type eq "unknown") {
		return "rgb(150,150,150)";
	}
	if (defined $type and $type eq "gc") {
		my $r = 200 + int(55 * $v1);
		my $x = 50 + int(80 * $v1);
		return "rgb($r,$x,$x)";
	}
	if (defined $type and $type eq "thread") {
		my $x = 175 + int(55 * $v1);
		my $b = 50 + int(20 * $v1);
		return "rgb($x,$x,$b)";
	}
