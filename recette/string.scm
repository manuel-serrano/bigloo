...
(define (test-string)
   ...
   (test "string>=?" (string<=? "012345" "012345") #t)
   (test "string-ci<?.1" (string-ci<? "abcdef" "ABCDEF") #f)
   (test "string-ci<?.2" (string-ci<? "abcdef" "ABCDEG") #t)
   (test "string-ci<=?.1" (string-ci<=? "A" "A") #t)
   (test "string-ci<=?.2" (string-ci<=? "abcdef" "ABCDEG") #t)
   (test "string-ci<=?.3" (string-ci<=? "abcdef" "ABCDEG") #t)
   (test "string-ci>?.1" (string-ci>? "abcdef" "ABCDEG") #f)
   (test "string-ci>?.2" (string-ci>? "abcdef" "ABCDEG") #f)
   (test "string-ci>=?.1" (string-ci>=? "abcdef" "ABCDEG") #f)
   (test "string-ci>=?.2" (string-ci>=? "abcdef" "ABCDEF") #t)
   (test "string-ci=?.1" (string-ci=? "toto" "titi") #f)
   (test "string-ci=?.2" (string-ci=? "toto" "Toto") #t)
   (test "string-ci=?.3" (string-ci=? "toto" "tot") #f)
   (test "substring=?.1" (substring=? "abcdef" "abcd" 3) #t)
   (test "substring=?.2" (substring=? "abcdef" "Abcd" 3) #f)
   (test "substring-ci=?.1" (substring-ci=? "abcdef" "abc" 4) #f)
   (test "substring-ci=?.2" (substring-ci=? "abcdef" "abcd" 3) #t)
   (test "substring-ci=?.3" (substring-ci=? "abcdef" "Abcd" 3) #t)
   (test "substring-ci=?.4" (substring-ci=? "abcdef" "Abc" 4) #f)
