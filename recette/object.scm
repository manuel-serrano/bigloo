...
(define (test-object)
   ...
   (test "field.1" (let loop ((fs (class-fields virtual-2)))
		      (cond
			 ((null? fs)
			  #f)
			 ...
			 (else
			  (loop (cdr fs)))))
	 #t)
   (test "field.2" (let loop ((fs (class-fields virtual-2)))
		      (cond
			 ((null? fs)
			  #f)
			 ...
			 (else
			  (loop (cdr fs)))))
	 #t)
   (test "field.3" (let loop ((fs (class-fields virtual-2)))
		      (cond
			 ((null? fs)
			  #f)
   ...
   (test "dump.c" (equal? (restore-obj "misc/dump.c") *dump-object*) #t)
   (test "dump.jvm" (equal? (restore-obj "misc/dump.jvm") *dump-object*) #t)
   (if (file-exists? "misc/dump.dotnet")
       (test "dump.dotnet" (equal? (restore-obj "misc/dump.dotnet") *dump-object*) #t))
