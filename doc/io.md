...
@deffn {bigloo procedure} open-input-descriptorOpen a file descriptor (as C @code{fdopen}).



@deffn {bigloo procedure} open-input-string@deffnx {bigloo procedure} open-input-string!
@cindex SRFI-6
...
@deffn {bigloo procedure} open-input-procedure
Returns an @code{input-port} able to deliver characters from
...
@deffn {procedure} open-output-fileThe same syntax as @code{open-input-file} for file names applies here.
When a file name starts with @samp{| }, Bigloo opens an output pipe
instead of a regular file.


@deffn {bigloo procedure} append-output-fileIf @code{file-name} exists, this function returns an @code{output-port}
on it, without removing it. New output will be appended to @var{file-name}.
If @code{file-name} does not exist, it is created.


@deffn {bigloo procedure} open-output-stringThis function returns an @emph{output string port}. This object has almost
...
@deffn {bigloo procedure} get-output-string
@cindex SRFI-6
...
@deffn {procedure} close-input-port
@deffnx {procedure} close-output-port
...
@deffn {procedure} closed-input-port
@deffnx {procedure} closed-output-port
